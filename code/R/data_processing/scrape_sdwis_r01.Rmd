...
violation_fresh %>% filter(VIOLATION.CONTAMINANT_CODE == "1005", 
                           VIOLATION.IS_HEALTH_BASED_IND == "Y") %>% 
  mutate(YEAR = year(VIOLATION.COMPL_PER_BEGIN_DATE)) %>% 
  group_by(VIOLATION.PRIMACY_AGENCY_CODE, YEAR) %>% 
  tally() %>% 
  ggplot(aes(x = YEAR, y = n, color = VIOLATION.PRIMACY_AGENCY_CODE)) +
  geom_line(size = 1.5) +
...
violation_fresh %>% filter(VIOLATION.CONTAMINANT_CODE == "1005", 
                           VIOLATION.IS_HEALTH_BASED_IND == "Y") %>% 
  group_by(VIOLATION.PRIMARY_SOURCE_CODE, year(VIOLATION.COMPL_PER_BEGIN_DATE)) %>% 
  tally()
