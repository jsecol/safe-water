...
locations_pts <- st_as_sf(locations_dat, coords = c("LON", "LAT"), crs = 4163) %>% 
  st_transform(crs = st_crs(st_crs(epa01_cosub2018)))
...
* pretty good, but 1 in MA put in 33:NH
...
unzip(zipdir1[i],
      exdir = paste0(tempdir1, "/", "folder", i),
      junkpaths = TRUE,
      overwrite = FALSE)
}


census_files <- dir_ls(tempdir1, 
                       recurse = TRUE, 
...
```{r}
plot(HC03_VC161 ~ HC01_VC85, joined_df1)

round(cor(joined_df1[, c("HC03_VC161", "HC01_VC85")], 
...
# #         between(year(VIOLATION.COMPL_PER_BEGIN_DATE), 2013, 2017)) %>% 
#   filter(VIOLATION.PWSID %in% locations_dat$PWSID, 
#          year(VIOLATION.COMPL_PER_BEGIN_DATE) <= 2013) %>% 
...
#### Maybe add to GitHub as geojson, so anyone can use it more easily?


```{r}

shape_combined_cdat1 <- left_join(shape_combined, census_dat1, 
                                  by = c("GEOID" = "GEO.id2"))


hist(shape_combined_cdat1$HC03_VC161)
...
plot(shape_combined_cdat1["HC03_VC161"], 
     breaks = c(0, 5, 10, 15, 20, 25, 
                max(r01_shape_cdat1$HC03_VC161, na.rm = TRUE)))

shape_combined %>%
  st_write("../../../data/geography/epa01_cosub2018.geojson")
...
shape_combined_mdat2 <- left_join(shape_combined, modeldata2, 
                                  by = "GEOID")


hist(shape_combined_mdat2$COLI_HB_prp)
...
plot(shape_combined_mdat2["COLI_HB_prp"])
...
shape_combined_mapdat1 <- joined_df1 %>% 
  ...
  left_join(shape_combined, ., by = "GEOID")
...
plot(shape_combined_mapdat1["COLI_HB_PRP"])
