...
zipdir <- dir_ls("zipped/tl", glob = "*.zip")

for(i in 1:length(zipdir)) {
unzip(zipdir[i],
      exdir = "unzipped/tl",
      junkpaths = TRUE,
      overwrite = FALSE)
...
# shapefile list
shpdir <- dir_ls("unzipped/tl", glob = "*.shp")
basename(shpdir)
...
locations <- drive_get("WaterSystem_Locations")
locations_dat <- read_sheet(locations, sheet = "Region01")
...
# do in github
joined_df %>% write_csv("processed/r01_pwsid_cosub.csv")
...
zipdir2 <- dir_ls("../../table", glob = "*.zip")

for(i in 1:length(zipdir2)) {
unzip(zipdir2[i],
      exdir = paste0("../../table/unzipped", "/", "folder", i),
      junkpaths = TRUE,
      overwrite = FALSE)
}


census_files <- dir_ls("../../table/unzipped", 
                       recurse = TRUE, 
