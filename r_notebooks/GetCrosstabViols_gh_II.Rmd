...
```{r results='hold'}
viol_HB_byYear <- viol_all %>% filter(PWS_ACTIVITY_CODE == "A",
                    between(cpbd, as.Date("2008-01-01"), as.Date("2017-12-31"))) %>% 
  mutate(cpbd_year = year(cpbd))
...
viol_HB_byYear_x %>% 
  write.csv(gzfile("data_export/viol_HB_byYear0817_x.csv.gz"), row.names = FALSE)
...
+ join to zip code-associated data, and some others back from Violations
...
viol_all_A_unique <- viol_all %>% filter(PWS_ACTIVITY_CODE == "A") %>% 
  select(PWSID, POPULATION_SERVED_COUNT, PRIMACY_AGENCY_CODE) %>% 
  unique()

viol_RTC_byYear_x_c <- left_join(viol_RTC_byYear_x_c, viol_all_A_unique, by = "PWSID")
...
+ some exploration of State, etc. 

1st how State from zipcode R package may differ from SDWIS for some PWSIDs


```{r}

viol_RTC_byYear_x_c %>% filter(PWS_TYPE_CODE == "CWS", Rzcpkg_state != PRIMACY_AGENCY_CODE)

viol_RTC_byYear_x_c %>% filter(PWS_TYPE_CODE == "CWS", Rzcpkg_state != PRIMACY_AGENCY_CODE) %>% 
  select(Rzcpkg_state, PRIMACY_AGENCY_CODE) %>% distinct()
...
*roadblock: zip code can be deceptive, here a PWSID in FL (FL3490293) with an AZ address:*  
...
viol_RTC_byYear_x_c %>% 
  filter(Rzcpkg_state %in% c(state.abb, "PR"), PWS_TYPE_CODE == "CWS") %>% 
  group_by(Rzcpkg_state) %>% 
  summarise_at(vars(RTC_HB_Y_2008:RTC_HB_Y_2012), sum) %>% 
  print.data.frame()

viol_RTC_byYear_x_c %>% 
  filter(Rzcpkg_state %in% c(state.abb, "PR"), PWS_TYPE_CODE == "CWS") %>% 
  group_by(Rzcpkg_state) %>% 
  summarise_at(vars(RTC_HB_Y_2013:RTC_HB_Y_2017), sum) %>% 
  print.data.frame()

viol_RTC_byYear_x_c %>% 
  filter(Rzcpkg_state %in% c(state.abb, "PR"), PWS_TYPE_CODE == "CWS") %>% 
  group_by(Rzcpkg_state) %>% 
  summarise_at(vars(RTC_HB_Y_2008:RTC_HB_Y_2017), sum) %>% 
  tidyr::gather(Variable, Value, 2:11) %>% 
  arrange(Rzcpkg_state, Variable) %>% 
  ggplot(aes(x = Variable, y = Value, group = Rzcpkg_state)) +
  geom_line(aes(color = Rzcpkg_state)) +
...
viol_RTC_byYear_x_c %>% 
  ...
  tidyr::gather(Variable, Value, 2:11) %>% 
  arrange(Rzcpkg_state, Variable) %>% 
  ggplot(aes(x = Variable, y = Value+1, group = Rzcpkg_state)) +
  geom_line(aes(color = Rzcpkg_state)) +
  scale_y_log10() +
  theme(legend.position = "none")
...
#### Actual numbers roadblock

Ex., MO4010718 in MO has numerous HB RTC COLIFORM (TCR) in 2014:
  
https://ofmpub.epa.gov/enviro/sdw_report_v3.first_table?pws_id=MO4010718&state=MO&source=Ground%20water%20purchased&population=360&sys_num=0  
  
not sure if should standardize somehow/reduce to 1, ideally might want to generate complete timeline relevant to a particular contaminant for each PWSID  
...
```{r}
viol_RTC_byYear_x_c <- viol_RTC_byYear_x_c %>% 
  mutate(RTC_HB_N = ifelse(RTC_HB_N_sum > 0, 1, 0), 
         RTC_HB_Y = ifelse(RTC_HB_Y_sum > 0, 1, 0))
```
...
viol_RTC_byYear_x_c %>% 
  filter(Rzcpkg_state %in% c(state.abb, "PR"), 
         PWS_TYPE_CODE == "CWS") %>% 
  group_by(Rzcpkg_state) %>% 
  summarise(RTC_HB_Y_sum = sum(RTC_HB_Y)) %>% 
  left_join(., Total_CWS_byState, by = "Rzcpkg_state") %>% 
  ggplot(aes(x = reorder(Rzcpkg_state, (RTC_HB_Y_sum / totCWS), sum), 
             y = (RTC_HB_Y_sum / totCWS)*100)) + 
  geom_bar(stat = "identity") + 
  ylab("% of total CWS") +
  geom_text(aes(y = 0, label = totCWS), size = 3, nudge_y = -2) + 
  coord_flip(ylim = c(-1, 60)) + 
  ggtitle("State/Territory % of Active, CWS's that returned to compliance \n for a health-based COLIFORM (TCR) violation 2008-17", 
          subtitle = "(Total Active, CWS N per State/Territory by label)")
...
*Maybe interesting to focus on NE and MO, quite different sample sizes but high*
