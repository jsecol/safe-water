...
1. Convert to date fields

```{r}

violations <- violations %>% 
  mutate(compl_per_begin_date = dmy(compl_per_begin_date), 
         compl_per_end_date = dmy(compl_per_end_date), 
         rtc_date = dmy(rtc_date))
...
2. focus on one contaminant

```{r}

viol_coli <- violations %>% filter(contaminant_code == 3100, 
                                   pwsid %in% locations$pwsid)

table(viol_coli$primacy_agency_code)

summary(viol_coli$compl_per_begin_date)

```

+ tallying health-based by primary source code
```{r}

viol_coli %>% filter(is_health_based_ind == "Y") %>% 
  group_by(primacy_agency_code, primary_source_code) %>% 
  tally() %>% tidyr::spread(primary_source_code, n)
...
viol_coli_sp <- viol_coli %>% 
  filter(is_health_based_ind == "Y") %>% 
  group_by(pwsid) %>% 
  tally() %>% inner_join(., locations, by = "pwsid")
  
ggplot(states, aes(x = long, y = lat, group = group)) + 
  geom_polygon(fill = NA, color = "black") + 
  geom_point(data = viol_coli_sp, 
             aes(x = LON, y = LAT), 
             alpha = 0.2, color = "red", inherit.aes = FALSE) + 
  coord_fixed(1.2) + 
  ggtitle("Water systems with coliform health-based violations") + 
...
ggplot(states, aes(x = long, y = lat, group = group)) + 
  geom_polygon(fill = NA, color = "black") + 
  geom_pointdensity(data = viol_coli_sp, 
                    aes(x = LON, y = LAT),
                    stat = "pointdensity",
                    position = "identity", 
                    adjust = 0.5,
...
## III - Get environmental data

+ Extreme Precipitation in New York & New England [ http://www.precip.net ]

```{r}
# change path eventually, google drive?
library(raster)

rpe_1d_1y <- raster("C:/GIS/raster/asc/rpe_1d_1y.asc")
rpe_1d_1y

library(sf)
ne_shape <- 
  st_read("C:/GIS/raster/asc/NE_states_counties_WGS72/NE_states_counties_WGS72.shp", 
          stringsAsFactors = FALSE) %>% 
  filter(TRUE)

crs(rpe_1d_1y) <- crs(ne_shape)
crs(rpe_1d_1y)
...
plot(ne_shape[ne_shape$STATE %in% 
                c("CT", "MA", "ME", "NH", "RI", "VT"), "STATE_ABBR"], 
     add = TRUE, col = NA)
...
```{r}
locations_rpe_1d_1y <- extract(x = rpe_1d_1y, 
                               y = locations[, c("LON", "LAT")])

locations_rpe_1d_1y[1:10]
summary(locations_rpe_1d_1y)
sum(is.na(locations_rpe_1d_1y))

# join data
locations_rpe_1d_1y <- tibble(pwsid = locations$pwsid, 
                              rpe_1d_1y = locations_rpe_1d_1y)
...
2. Get 5-year coliform health-based violation history

+ restrictive analysis at this point, don't care about non-health based but could add it in later

```{r}

pwsids_coli_5y <- viol_coli %>% 
  filter(pwsid %in% locations$pwsid) %>% 
  filter(pwsid %in% pwsids_2012$pwsid) %>%   
  mutate(cpbd_year = year(compl_per_begin_date)) %>% 
  dplyr::select(pwsid, cpbd_year) %>% distinct() %>% 
  filter(cpbd_year >= 2012) %>% 
  mutate(value = 1) %>% 
...
+ hurdle 2: whoa, too few in 2017!!

```{r}

sapply(pwsids_coli_5y[, 2:7], function(x) sum(x))

```

+ solution 2: go further back and model 2014 as outcome (just a shot in the dark picking 2014, but hopefully data are closer to reality) - NOTE could use 2008 (see graph below)


```{r}

pwsids_2009 <- violations %>% 
  filter(pwsid %in% locations$pwsid) %>% 
  mutate(cpbd_year = year(compl_per_begin_date)) %>% 
  dplyr::select(pwsid, cpbd_year) %>% distinct() %>% 
  filter(cpbd_year <= 2009) %>% 
  mutate(value = 1) %>% 
  tidyr::spread(key = cpbd_year, value = value, fill = 0, sep = "_")

pwsids_coli_5y_2009 <- viol_coli %>% 
  filter(pwsid %in% pwsids_2009$pwsid) %>%   
  mutate(cpbd_year = year(compl_per_begin_date)) %>% 
  dplyr::select(pwsid, cpbd_year) %>% distinct() %>% 
  filter(cpbd_year >= 2009) %>% 
  mutate(value = 1) %>% 
...
sapply(pwsids_coli_5y_2009[, 2:10], function(x) sum(x))
...
violations %>% 
  ...
  group_by(cpbd_year) %>% 
  tally() %>% 
  ggplot(aes(x = cpbd_year, y = n)) +
  geom_bar(stat = "identity", fill = "grey80", color = "grey20") + 
  scale_x_continuous(breaks = 1990:2018, limits = c(1990, 2019)) +
  ggtitle("Health-based coliform violations pwsid count by year 1990-2018") + 
...
glm1 <- glm(cpbd_year_2014 ~ cpbd_year_2013, 
            family = binomial(), 
            data = pwsids_coli_5y_2009)
summary(glm1)

table(pwsids_coli_5y_2009$cpbd_year_2014, 
      pwsids_coli_5y_2009$cpbd_year_2013, 
      deparse.level = 2)
...
glm2 <- locations %>% 
  inner_join(., water_systems, by = "pwsid") %>% 
  inner_join(., locations_rpe_1d_1y, by = "pwsid") %>% 
  inner_join(., pwsids_coli_5y_2009, by = "pwsid") %>% 
  glm(cpbd_year_2015 ~ cpbd_year_2014 + rpe_1d_1y + pws_type_code.x, 
            family = binomial(), 
            data = .)
...
+ solution 2: just model single year (2017 for starters) and ignore prior history altogther (use spatial data/get counties and use county-level data etc.)
