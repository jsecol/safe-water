...
z_NH_centers_ll <- st_transform(NH_centers, crs = 4326)

st_crs(z_NH_centers_ll)

z_NH_centers_ll_data <- cbind(st_drop_geometry(NH_centers_ll), 
                            st_coordinates(NH_centers_ll))

z_NH_centers_ll_data$NAME <- tolower(z_NH_centers_ll_data$NAME)
...
watsys_ZCCTservedloc_II %>% filter(primacy_agency_code.x == "NH") %>% 
  left_join(., z_NH_centers_ll_data, by = c("city_served" = "NAME")) %>% 
  select(pwsid, primacy_agency_code.x, primacy_type, primary_source_code, pws_name,
         pws_type_code, service_connections_count, population_served_count, 
         fac_name:fac_accuracy_meters, area_type_code.x, zip_code_served,
