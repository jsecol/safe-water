...
## now for other coordinate sources **stop this except maybe for stats**
...
*(WAIT) exclude ECHO already done, as good or maybe better*

```{r}
# watsys_noecholoc_NewEng_gaZC <- anti_join(watsys_NewEng_gaZC, 
#                                           watsys_echoloc_I_NewEng, 
#                                           by = "pwsid")
```
...
#   write_csv(., "data_export/watsys_noecholoc_NewEng_gaZC.csv")
...
*(WAIT) exclude ECHO already done, maybe better*
```{r}
...
set_II <- c(set_I, watsys_noecholoc_NewEng_NHgisI_2$pwsid)

length(set_II) == length(unique(set_II))

```


```{r}
watsys_noecholoc_NewEng_nogeo <- water_system %>% 
  filter(epa_region == "01", !pwsid %in% set_II)

# print from console to get simple rst text format (reStructuredText simple table)
knitr::kable(table(watsys_noecholoc_NewEng_nogeo$primacy_agency_code, 
      watsys_noecholoc_NewEng_nogeo$pws_type_code), format = "rst")
...
watsys_noecholoc_NewEng_gaCT2_stillneed1 <- 
  watsys_noecholoc_NewEng %>% 
  filter(!pwsid %in% set_II) %>% 
  inner_join(., ga_NewEng_CT_1x_df, by = "pwsid")
