...
#### doing state by state GIS in another rmd

*State = NH*

```{r message=FALSE}

z_NH_centers_ll_data <- 
  read_csv("C:/Users/Jim/Files/CFB/GIS/locations/NH/z_NH_centers_ll_data.csv")
...
*using fixed names, see _ME.Rmd*
```{r}
ga_r01_CT_1x_df_MEgisI <- ga_r01_CT_1x_df %>% 
  filter(primacy_agency_code == "ME") %>% 
  left_join(., ME_towns_pts_coords, by = c("city_served2" = "sdwis_name_match"))
...
ga_r01_CT_1x_df_VTgisI <- ga_r01_CT_1x_df %>% 
  ...
  left_join(., VT_towns_ctrs_coords, by = c("city_served2" = "sdwis_match"))
