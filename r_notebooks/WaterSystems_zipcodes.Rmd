...
```{r}
merge1_A %>% select(PWSID, PWS_TYPE_CODE, ZIP_CODE5, LAT, LON, COORD_SRC) %>% 
...
+ get data for the zip codes (note can be more involved, use buffer etc; see ?extract)
```{r}
...
# data uses a scale factor of 10, so to get degrees C:
...
CWS_A_sp_main@data %>% 
  ...
  geom_point(size = 0.8) + 
  scale_color_viridis() +
  ggtitle("CWS zip codes: mean annual temperature (C)")
