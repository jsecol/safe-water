...
#### TEMP
# load("C:/Users/Jim/GitHub/safe-water/r_notebooks/WaterSystems_zipcodes.RData")
```
...
#### Trying to ID where wrong State, etc. Zip Codes using echo
...
#### most problematic ones, with PRIMACY_AGENCY_CODE not same as ADMIN address State
```{r}
merge1 <- merge1 %>% 
  mutate(geo_filter = case_when(is.na(echo_dat) & PRIMACY_AGENCY_CODE != STATE_CODE & 
         PRIMACY_AGENCY_CODE %in% c(state.abb, "DC", "PR") ~ "NoMatch", 
         echo_dat == "fac_zip used" ~ "EchoZip"))

merge1 %>% filter(geo_filter == "NoMatch") %>% count() %>% .$n
```
...
merge1 %>% filter(geo_filter == "EchoZip") 
...
```{r}
merge1 <- left_join(merge1, zcta, by = c("fac_zip" = "GEOID"))
...
```{r}
merge1 <- left_join(merge1, zipcode, by = c("fac_zip" = "zip"))
...
length(unique(merge1_A$fac_zip))
...
#### Adding in merge 2 city-associated values
```{r} 
...
```{r}
merge1 %>% select(PWSID:PWS_NAME, PWS_TYPE_CODE, 
