...
my_matching_A <- read_csv("data_export/PWSID_coordinates.csv.gz")
my_matching_A$PWS_ACTIVITY_CODE <- "A"
```


```{r}

shape1_dat <- left_join(my_matching_A[, c(1:10, 12, 22)], ws4, 
...
# check on "A"
table(shape1_dat$PWS_ACTIVITY_CODE)

# check on "CWS", etc.
table(shape1_dat$PTC_CWS)
table(shape1_dat$PTC_NTNCWS)

table(shape1_dat$PWS_TYPE_CODE)
